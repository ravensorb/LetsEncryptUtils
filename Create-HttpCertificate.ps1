...
PROCESS
{
	...
	if ($PSCmdlet.ShouldProcess("$domainNames", "Creating actual Certificate")) {
		#New-PAHttpCertificate $domainNames -AcceptTOS -Install -Contact $contactEmail --FriendlyName $friendlyName -PluginArgs @{Path=$PluginArgs.path; UserName=$PluginArgs.userName; Password=$PluginArgs.password} -PfxPass $pfxPassword -WhatIf:$WhatIfPreference -Verbose:$VerbosePreference
	}
}
END
{
